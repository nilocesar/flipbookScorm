...
--------------------------*/
    ...
    $(document).ready(function (e) {
        var config = $('#fb5-ajax').data('config');
        $(window).load(
            function () {
                //
                $.getJSON(config['config'], function (json) {
                    __config = json;
                })
                    .success(function () {})
                    .error(function () {
    ...
    createPagesAjax = function () {
        __nPagina = __config.numeros_paginas;
        document.title = __config.titulo;
    ...
    multiplePageAjax = function (pageID) {
        var __currentPage = null;
        var pagelink = "";

        if (pageID == -1) {
            pagelink = "pages/preloader.html";
        } else if (pageID == 0) {
            pagelink = "pages/begin.html";
        } else if (pageID > 0 && pageID <= __nPagina) {
            pagelink = "pages/pagina" + pageID + ".html";
        } else if (pageID == __nPagina + 1) {
            pagelink = "pages/footer.html";
        }
        ...
        $.ajax({
            ...
            error: function () {
                pageErro(pageID, __currentPage);
                console.log("error de carregamento de página index ID::  " + pageID);
            },
            complete: function () {
                pageSucess(pageID, __currentPage);
            }
        });
    };

    //TODO:
    pageSucess = function (pageID, __currentPage) {
        if (pageID == -1) {
            $("#fb5-ajax").append("<div data-current='flipage' class='fb5' id='fb5'></div>");
            $("#fb5").attr("data-current", __config.id)
            $("#fb5").append(__currentPage);

            multiplePageAjax(pageID + 1);
        } else if (pageID == 0) {
            $("#fb5").append(" <div id='fb5-container-book'></div>");
            $("#fb5-container-book").append(__currentPage);
            $("#fb5-container-book").append(" <div id='fb5-book'></div>");

            //
            multiplePageAjax(pageID + 1);
        } else if (pageID > 0 && pageID <= __nPagina) {
            $("#fb5-book").append(__currentPage);

            //
            multiplePageAjax(pageID + 1);
        } else if (pageID == __nPagina + 1) {
            $("#fb5-container-book").append("<a class='fb5-nav-arrow prev'></a>");
            $("#fb5-container-book").append("<a class='fb5-nav-arrow next'></a>");
            $("#fb5").append(__currentPage);
    ...
    pageErro = function (pageID, __currentPage) {
        if (pageID == -1) {
            $("#fb5-ajax").append("<div data-current='flipage' class='fb5' id='fb5'></div>");
            $("#fb5").attr("data-current", __config.id)
            $("#fb5").append("<h1> Erro de carregamento dessa página</h1>");

            //
            multiplePageAjax(pageID + 1);
        } else if (pageID == 0) {
            $("#fb5").append(" <div id='fb5-container-book'></div>");
            $("#fb5-container-book").append("<h1>Erro de carregamento dessa página</h1>");
            $("#fb5-container-book").append(" <div id='fb5-book'></div>");

            //
            multiplePageAjax(pageID + 1);
        } else if (pageID > 0 && pageID <= __nPagina) {
            $("#fb5-book").append("<h1>Erro de carregamento dessa página</h1>");
            multiplePageAjax(pageID + 1);
        } else if (pageID == __nPagina + 1) {
            $("#fb5-container-book").append("<a class='fb5-nav-arrow prev'></a>");
            $("#fb5-container-book").append("<a class='fb5-nav-arrow next'></a>");
    ...
    currentPageInicialScorm = function () {
        $('body').trigger('pageInicialScorm');
    }
...
--------------------------*/
    var Book_v5 = {
        ...
        load: function () {
            ...
            //Controle Scorm Iniciar Page
            currentPageInicialScorm();
            //
        },
